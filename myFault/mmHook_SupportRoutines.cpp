...
PPTE __stdcall GetPteAddress(void *VirtualAddress)
{
	PPTE pPTE = 0; // [sp+Ch] [bp-4h]@1

	_disable();
	if (*(char*)(4 * ((unsigned long)VirtualAddress >> 22) - 1070596096) & 0x80)
		*pPTE = 4 * ((unsigned long)VirtualAddress >> 22) - 1070596096;
	else
		*pPTE = 4 * ((unsigned long)VirtualAddress >> 12) - 1073741824;
	_enable();
	return pPTE;
}
...
bool __stdcall IsInLargePage(void *VirtualAddress)
{
	bool LargePage; // [sp+Fh] [bp-1h]@1
...
void __stdcall DisableGlobalPageFeature(PPTE pPte)
{
	*pPte |= 0xFFFFFEFFu;
}
...
void __stdcall EnableGlobalPageFeature(PPTE pPte)
{
	*pPte |= 0x100u;
}
...
void __stdcall MarkPageNotPresent(PPTE pPte)
{
	*pPte &= 0xFFFFFFFEu;
}
...
void __stdcall MarkPagePresent(PPTE pPte)
{
	*pPte |= 1u;
}
...
void __stdcall MarkPageReadWrite(PPTE pPte)
{
	*pPte |= 2u;
}
...
void __stdcall MarkPageReadOnly(PPTE pPte)
{
	*pPte |= 0xFFFFFFFDu;
}
...
ULONG __stdcall GetPhysicalFrameAddress(PPTE pPte)
{
	ULONG Frame = 0; // [sp+Ch] [bp-4h]@1
	return *pPte >> 12;
}
...
void __stdcall ReplacePhysicalFrameAddress(PPTE pPte, ULONG* oldFrame, ULONG newFrame)
{
	*oldFrame = GetPhysicalFrameAddress(pPte);
	*pPte = *pPte & 0xFFF | newFrame & 0xFFFFF000;//?
}
